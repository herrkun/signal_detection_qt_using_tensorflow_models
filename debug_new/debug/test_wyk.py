...
#输入层、输出层权重、偏置
    ...
    with tf.name_scope('lstm_input'):        
        input_x=tf.reshape(X,[-1,input_size])  #需要将batch*time_step*features转成2维(batch*time_step)*features进行计算
        input_rnn=tf.nn.relu(tf.matmul(input_x,w_in)+b_in) #输入数据全连接转为隐藏层的维度，计算后的结果作为隐藏层的输入
        input_rnn_r=tf.reshape(input_rnn,[-1,time_step,rnn_unit])  #再将数据转换回3维batch*time_step*features，作为lstm cell的输入
...
with tf.name_scope('learning_rate'): 
    learning_rate = tf.train.exponential_decay(lr,global_step,batch_index_train,decay_rate, staircase=True)
...
with open(r'E:\wyk\another_pci_wyk\debug_new\debug\epoch.txt','w') as f:
    f.write(str(0)+' / '+str(0))
...
def reload_model(filepath,modelfile,pre_mode='no',lvbo_mode='no',lvbo1=8,low=0,high=0,wave1='sym5',wave2=3):
    ...
    np.save(model_parfile+'\\signal_wyk_withgraph.npy',signal)   
