...
def reload_model(filepath,modelfile,pre_mode='no',lvbo_mode='no',lvbo1=8,low=0,high=0,wave1='sym5',wave2=3):
    ...
    for i in range(800,1000):
        ...
        for j in range(batch_size_set):
            test_x.append(volt_data2[(i*batch_size_set+j):(i*batch_size_set+j+time_step_set)])
        ...
        if i%100==0:            
            print("The fin_signal_feature_predict batch_num:",i)        
    ...
    filepath=r'E:\wyk\2019newmodel\fin_km_centers.h5'
    ...
    np.save(file_parfile+'\\distance_wyk_nograph.npy',final_distance) 
    
    distance_wyk=final_distance.max(axis=1)
    np.save(file_parfile+'\\final_distance_wyk_nograph.npy',distance_wyk)
